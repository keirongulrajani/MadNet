...
describe('StakeNFTDescriptor: Tests StakeNFTDescriptor methods', async () => {
  ...
  beforeEach(async function () {
    const StakeNFTDescriptorLibFactory = await ethers.getContractFactory(
      'StakeNFTDescriptor'
    )
    const stakeNFTDescriptorLib = await StakeNFTDescriptorLibFactory.deploy()
    await stakeNFTDescriptorLib.deployed()

    const StakeNFTDescriptorFactory = await ethers.getContractFactory(
      'StakeNFTDescriptorMock',
      {
        libraries: {
          StakeNFTDescriptor: stakeNFTDescriptorLib.address
        }
      }
    )
